...
# 京东到家鲜豆任务 自动完成鲜豆任务
0 8 * * * node /scripts/jd_dj_bean.js >> /scripts/logs/jddj_bean.log 2>&1
# 京东到家鲜豆庄园 自动完成庄园任务自动浇水
0 9 * * * node /scripts/jd_dj_plantBeans.js >> /scripts/logs/jddj_plantBeans.log 2>&1
# 鲜豆庄园领水滴 定时领取庄园水滴
0-25/5 0 * * * node /scripts/jd_dj_getPoints.js >> /scripts/logs/jddj_getPoints.log 2>&1
